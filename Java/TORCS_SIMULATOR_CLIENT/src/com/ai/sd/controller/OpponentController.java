...
public class OpponentController {
	...
	private static double minSomeoneInfron(MySensorModel sensors, boolean exact){
		...
		for(int i = 15; i<=21; ++i){
			...
			double width = Math.abs(SDDriverHelper.sinAngO[i]*opponentInfo[i]);
									
			if(width < carWidth/2.0){//it is infront of us
				
				double distance = Math.abs(SDDriverHelper.cosAngO[i]*opponentInfo[i]);
				if(distance < minDist)
					minDist = distance;
			}else{
				if(!exact){
					...
					double distance = Math.abs(SDDriverHelper.cosAngO[i]*opponentInfo[i])*(1.0-c)*5.0;
					if(distance < minDist)
						minDist = distance;
				}
			}
